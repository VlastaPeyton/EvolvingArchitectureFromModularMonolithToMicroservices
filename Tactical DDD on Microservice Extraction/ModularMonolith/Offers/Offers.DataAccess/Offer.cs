...
namespace Offers.DataAccess
{
    public sealed class Offer
    {
        ...
        public static Offer PrepareStandardPassExtension(Guid customerId, DateTimeOffset now)
        {
            const decimal standardDiscount = 0.1m;

            var offer = new Offer(Guid.NewGuid(), customerId, now, standardDiscount, now, now.AddYears(1));
